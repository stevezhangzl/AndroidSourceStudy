...
bool JavaVMExt::LoadNativeLibrary(JNIEnv* env,
  ...
  Thread* self = Thread::Current();
  {
    ...
    MutexLock mu(self, *Locks::jni_libraries_lock_);
    library = libraries_->Get(path); //首先判断是否已经加载过这个library
  }
  ...
  if (library != nullptr) {
    ...
    if (library->GetClassLoaderAllocator() != class_loader_allocator) { //判断之前加载的library 时使用的class loader和当前的class loader是不是一致的
      // The library will be associated with class_loader. The JNI
      // spec says we can't load the same library into more than one  //不满足java规范
      // class loader.
  ...
  bool created_library = false;
  {
    ...
    MutexLock mu(self, *Locks::jni_libraries_lock_); //可能多线程都尝试打开library
