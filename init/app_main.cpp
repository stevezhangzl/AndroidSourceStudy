//https://cs.android.com/android/platform/superproject/+/android-7.0.0_r1:frameworks/base/cmds/app_process/app_main.cpp?hl=zh-cn
...
public:
    ...
    virtual void onZygoteInit()
    {
        sp<ProcessState> proc = ProcessState::self();
        ALOGV("App process: starting thread pool.\n");
        proc->startThreadPool(); //开启Binder线程池  以保证 其他 进程可以正确访问到Zygote所提供的服务
    }
...
int main(int argc, char* const argv[])
{
    ...
    AppRuntime runtime(argv[0], computeArgBlockSize(argc, argv));  //Android运行时环境
    ...
    while (i < argc) {
        ...
        if (strcmp(arg, "--zygote") == 0) {  //当前进程是否用于承载 zygote
            zygote = true;
            niceName = ZYGOTE_NICE_NAME;
        } else if (strcmp(arg, "--start-system-server") == 0) {  //是否需要启动system server
            startSystemServer = true;
        } else if (strcmp(arg, "--application") == 0) { //启动进入独立的程序模式
            application = true;
        } else if (strncmp(arg, "--nice-name=", 12) == 0) {  //此进程的“别名”
    ...
    if (zygote) {
        runtime.start("com.android.internal.os.ZygoteInit", args, zygote); //1.start
    } else if (className) {
