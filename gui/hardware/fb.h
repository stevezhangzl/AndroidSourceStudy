...
typedef struct framebuffer_device_t {
    ...
    //framebuffer的像素格式 如HAL_PIXEL_FORMAT_RGB_888等
    const int       format;
    ...
    //屏幕的每秒刷新频率。假如无法从设备获取这个值，Android系统会默认设置为60Hz
    const float     fps;

    /* min swap interval supported by this framebuffer */
    //该framebuffer支持的最小和最大的缓冲交换时间
     ...
     * 设置刷新区域，需要framebuffer驱动支持 在这个区域外的数据很可能被认为无效。
     */
     ...
     * Returns 0 on success or -errno on error.
     * 
     * 将buffer数据post到显示屏上。要求buffer必须与屏幕尺寸一致，并且没有被locked。这样buffer
     *内容将在下一次VSYNC中被显示出来。
     */
