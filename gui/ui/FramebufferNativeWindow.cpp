...
class NativeBuffer final: public ANativeObjectBase<
        ANativeWindowBuffer,
        NativeBuffer,
    ...
    : BASE(), fbDev(0), grDev(0), mUpdateOnDemand(false)
{
    ...
    if (hw_get_module(GRALLOC_HARDWARE_MODULE_ID, &module) == 0) { //加载模块
        int err;
        int i;
        err = framebuffer_open(module, &fbDev); //打开fb设备  在fb.h中
        ALOGE_IF(err, "couldn't open framebuffer HAL (%s)", strerror(-err));

        err = gralloc_open(module, &grDev);  //打开gralloc设备 在gralloc.h
        ...
        if(fbDev->numFramebuffers >= MIN_NUM_FRAME_BUFFERS &&
           fbDev->numFramebuffers <= MAX_NUM_FRAME_BUFFERS){
            mNumBuffers = fbDev->numFramebuffers; //根据fb设备中取值 获得buffer数
        } else {
            mNumBuffers = MIN_NUM_FRAME_BUFFERS;  //否则就采用最少的buffer数值 2
        }
        mNumFreeBuffers = mNumBuffers; //可用的buffer的个数，初始时是所有buffer可用
        mBufferHead = mNumBuffers-1;  //指向下一个将被申请的buffer(不是下一个可用的buffer)
        ...
        *((uint32_t *)&fbDev->format) = FRAMEBUFFER_FORCE_FORMAT;
        ...
        for (i = 0; i < mNumBuffers; i++) { //给每个buffer初始化
            buffers[i] = new NativeBuffer( //通过new创建实例
                    static_cast<int>(fbDev->width), //宽 
                    static_cast<int>(fbDev->height), //高
                    fbDev->format, GRALLOC_USAGE_HW_FB); //格式 这些来自fb设备
        }

        for (i = 0; i < mNumBuffers; i++) { //给每个buffer分配空间
            err = grDev->alloc(grDev, //alloc  这里是通过调用HAL的Galloc来申请内存
                    static_cast<int>(fbDev->width),
                    static_cast<int>(fbDev->height),
                    fbDev->format, GRALLOC_USAGE_HW_FB,  //用于framebuffer设备
...
/**
 * 核心：OPENGL ES 去过它个来分配一个可用于渲染的缓冲区
*/
int FramebufferNativeWindow::dequeueBuffer(ANativeWindow* window,
        ANativeWindowBuffer** buffer, int* fenceFd)
{
    //1, 先将入参中的ANativeWindow类型转成FramebufferNativeWindow  
    FramebufferNativeWindow* self = getSelf(window);
    //2,自动Mutex锁 
    Mutex::Autolock _l(self->mutex);

    //3, 计算mBufferHead
    int index = self->mBufferHead++; //首先index得到的是mBufferHead的当前位置，然后mBufferHead才加1
    if (self->mBufferHead >= self->mNumBuffers) //循环使用 ，大于等于就归0  所以mBufferHead永远是[0-2]中的一个
        self->mBufferHead = 0;

    // wait for a free non-front buffer
    //4，如果当前没有可用缓冲区
    while (self->mNumFreeBuffers < 2) {
        self->mCondition.wait(self->mutex); //没有就要等待释放，这里使用Condition同步机制
    }
    ALOG_ASSERT(self->buffers[index] != self->front, "");

    // get this buffer
    //5,如果有人释放了缓冲区  可用bufer数 -1 由于mBufferHead已经做过自增处理
