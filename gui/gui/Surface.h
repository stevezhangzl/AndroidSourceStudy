 ...
 */
class Surface
    : public ANativeObjectBase<ANativeWindow, Surface, RefBase> //继承ANativeWindow
{
public:
...
private:
    ...
    */
    ...
    //Surface内部用于存储buffer的地方， NUM_BUFFER_SLOTS 最多可达32个 用户dequeBuffer时才会真正分配空间
    BufferSlot mSlots[NUM_BUFFER_SLOTS];

    // mReqWidth is the buffer width that will be requested at the next dequeue
    // operation. It is initialized to 1.
    uint32_t mReqWidth; //下一次dequeue将会申请的尺寸 初始值是1
    ...
    PixelFormat mReqFormat; //下次申请的buffer的像素格式，初始是PIXEL_FORMAT_RGBA_8888

    // mReqUsage is the set of buffer usage flags that will be requested
    // at the next deuque operation. It is initialized to 0.
    uint32_t mReqUsage; //下次dequeue时会指定的usage类型
    ...
    Rect mCrop;  //修剪，下次queue时用于修剪缓冲区  可以调用setCrop来设置具体的值

    // mScalingMode is the scaling mode that will be used for the next
    // buffers that get queued. It is set by calling setScalingMode.
    int mScalingMode; //下次queue时对缓冲区进行scale  通过调用setScallingMode来设置具体的值

    // mTransform is the transform identifier that will be used for the next
    // buffer that gets queued. It is set by calling setTransform.
    uint32_t mTransform; //下次queue时图形翻转
    ...
    uint32_t mDefaultWidth;  //默认情况下的缓冲区的宽高值
    ...
    uint32_t mUserWidth;  //不为0的话，就是应用层指定的值，会覆盖前面的mDefaultWidth
    ...
    sp<GraphicBuffer>           mLockedBuffer;  //这三个需要锁保护
