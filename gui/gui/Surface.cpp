...
int Surface::dequeueBuffer(android_native_buffer_t** buffer, int* fenceFd) {
    ...
    { //只对{} 代码块自动上锁，学会这个技巧 不影响下面函数里再上次上锁
        Mutex::Autolock lock(mMutex);

        //1，宽高计算，图形缓冲区有一定的宽高属性，具体的值由mReqWidth或者mUserWidth决定
        reqWidth = mReqWidth ? mReqWidth : mUserWidth; //前者优先级高于后者
    ...
    int buf = -1;
    //2,dequeueBuffer 得到一个缓冲区
    sp<Fence> fence;
    status_t result = mGraphicBufferProducer->dequeueBuffer(&buf, &fence, swapIntervalZero,
            reqWidth, reqHeight, reqFormat, reqUsage); //生产者发挥作用
