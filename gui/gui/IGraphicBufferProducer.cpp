...
public:
    ...
    //第二个是出参，是本地进程的内存
    virtual status_t requestBuffer(int bufferIdx, sp<GraphicBuffer>* buf) { //参数有两个
        Parcel data, reply;
        ...
        if (nonNull) {
            *buf = new GraphicBuffer(); //生成一个GraphicBuffer 是一个本地实例
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch(code) {
        case REQUEST_BUFFER: {
            ...
            int bufferIdx   = data.readInt32(); //读取处理的Slot序号
            sp<GraphicBuffer> buffer;
            int result = requestBuffer(bufferIdx, &buffer); //调用本进程的实现
            reply->writeInt32(buffer != 0); //这里写入的是bool
            if (buffer != 0) {
                reply->write(*buffer); //真正写入的值
            }
